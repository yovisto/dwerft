...
public class PreProducerGenerator extends RdfGenerator {
	...
	private void createBacklinks(Element n, Resource r, Resource parent) {
		for (Mapping m : getMappings(n.getName(), MappingAction.LINK)) {
	...
	private Optional<Mapping> getConvertMapping(String input) {
		Optional<Mapping> o = getMappings(input, MappingAction.CONVERT).stream().findFirst();
		return o;
	}
	...
	private String evaluateContextMapAction(String input, String context) {
		String name = input;
		Optional<Mapping> o = getMappings(input, MappingAction.CONTEXTMAP).stream().filter(m -> StringUtils.equalsIgnoreCase(context, m.getContext())).findFirst();
		if (o.isPresent()) {
